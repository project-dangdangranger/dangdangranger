						...
						.then(async (data) => {
							...
							if (data.status === 200) {
								...
								try {
									await axios
										.get(
											`https://api.polygonscan.com/api?module=account&action=tokennfttx&contractaddress=${MINT_DOG_TOKEN_ADDRESS}&address=${walletAddress}&startblock=0&endblock=99999999&page=1&offset=100&sort=asc&apikey=${POLYGON_KEY}`,
										)
										.then(async (data) => {
											...
											if (data.status === 200) {
												...
												await axiosApi
													...
													.then(async (data) => {
														if (
															data.data.message === "강아지 프로필 등록 완료"
														) {
															Alert.alert(
																"프로필 생성이 완료되었습니다.",
